...
def main():
    ...
    print("Shortest word containing all vowels: {}".format(min([w for w in words if all(c in w.lower() for c in list("aeiou"))], key=len)))
    print("Words with 4 a's: {}".format([w for w in words if w.lower().count("a") == 4]))
    print("Words with 2+ q's: {}".format([w for w in words if w.lower().count("q") >= 2]))
